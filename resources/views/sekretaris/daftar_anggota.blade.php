...
@section('content_admin')
                    ...
                    <div class="card-body">
                        <div class="dt-responsive table-responsive">
                            ...
                            <table id="basic-btn-da" class="table table-striped table-bordered" style="width: 100%;">
                                ...
                                <tbody>
                                    @foreach ($anggota_sekretaris as $item)
                                        <tr>
                        ...
                        <tfoot>
                            <tr class="fw-bold bg-light">
                                ...
                                <th class="text-center"></th>
                                <th class="text-center"></th>
                                <th class="text-center">{{ $anggota_sekretaris->sum('umur') ?: '-' }}</th>
                                <th class="text-center"></th>
                                <th class="text-center"></th>
                                <th class="text-center"></th>
                                <th class="text-center"></th>
    ...
    @include('admin-temp.partials.import_modal', [
        'importRoute' => 'daftar_anggota.import_sekretaris',
        'title'       => 'Import Data Anggota',
        'columns'     => 'no_registrasi, name, jenis_kelamin, tempat_lahir, tanggal_lahir, umur, status_perkawinan, alamat, pendidikan, pekerjaan, role_pkk, keanggotaan_tp_pkk, kader_umum, kader_khusus, keterangan',
    ])

    <!-- Modal Tambah Anggota -->
    <div id="AddAnggotaModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="AddAnggotaModalTitle"
        aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document"> <!-- Tambah modal-lg biar agak lebar -->
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">Form Tambah Anggota PKK</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>

                <form action="{{ route('daftar_anggota.store_anggota_sekretaris') }}" method="POST"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" name="id_users" required>
                                    ...
                                    Looping data user dari controller di sini. Contoh:
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-warning text-dark">
                        <h5 class="modal-title">Form Update Anggota PKK</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>

                    <form action="{{ route('daftar_anggota.update_anggota_sekretaris', $item->id) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    <label class="form-label">role di PKK</label>
                                    <select class="form-select" name="role_pkk">
                                        <option value="Anggota" {{ $item->role_pkk == 'Anggota' ? 'selected' : '' }}>Anggota</option>
                                        <option value="Ketua" {{ $item->role_pkk == 'Ketua' ? 'selected' : '' }}>Ketua</option>
                                        <option value="Wakil" {{ $item->role_pkk == 'Wakil' ? 'selected' : '' }}>Wakil</option>
                                        <option value="Bendahara" {{ $item->role_pkk == 'Bendahara' ? 'selected' : '' }}>Bendahara</option>
                                        <option value="Sekretaris" {{ $item->role_pkk == 'Sekretaris' ? 'selected' : '' }}>Sekretaris</option>
                                        <option value="Pokja 1" {{ $item->role_pkk == 'Pokja 1' ? 'selected' : '' }}>Pokja 1</option>
                                        <option value="Pokja 2" {{ $item->role_pkk == 'Pokja 2' ? 'selected' : '' }}>Pokja 2</option>
                                        <option value="Pokja 3" {{ $item->role_pkk == 'Pokja 3' ? 'selected' : '' }}>Pokja 3</option>
                                        <option value="Pokja 4" {{ $item->role_pkk == 'Pokja 4' ? 'selected' : '' }}>Pokja 4</option>
                                    </select>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label">Dalam Keanggotaan TP PKK</label>
                                ...
                                <!-- Tempat Tanggal Lahir & Umur (MANUAL) -->
                                <div class="col-md-12 mb-3">
                                    <label class="form-label">Tempat Lahir</label>
                                    <input type="text" class="form-control" name="tempat_lahir"
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-danger text-white">
                        <h5 class="modal-title" id="DeletelayananModalTitle-{{ $item->id }}">Hapus Data Anggota</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ route('daftar_anggota.destroy_anggota_sekretaris', $item->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
