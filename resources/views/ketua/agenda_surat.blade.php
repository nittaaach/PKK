...
@section('content_admin')
    ...
    <div class="row"><div class="col-sm-12">
        <div class="card"><div class="card-header"><h5>Surat Masuk</h5></div><div class="card-body">
            <div class="dt-responsive table-responsive">
                <table id="surat-masuk-tbl" class="table table-striped table-bordered" style="width:100%;">
                    <thead><tr><th>NO</th><th>TGL TERIMA</th><th>TGL SURAT</th><th>NO SURAT</th><th>ASAL SURAT</th><th>PERIHAL</th><th>LAMPIRAN</th><th>DITERUSKAN KEPADA</th></tr></thead>
                    <tbody>
                        @foreach ($surat_masuk as $item)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $item->tanggal_terima ? \Carbon\Carbon::parse($item->tanggal_terima)->translatedFormat('d M Y') : '-' }}</td>
                            <td>{{ $item->tanggal_surat ? \Carbon\Carbon::parse($item->tanggal_surat)->translatedFormat('d M Y') : '-' }}</td>
                            <td>{{ $item->no_surat ?? '-' }}</td>
        ...
        <div class="card mt-3"><div class="card-header"><h5>Surat Keluar</h5></div><div class="card-body">
            <div class="dt-responsive table-responsive">
                <table id="surat-keluar-tbl" class="table table-striped table-bordered" style="width:100%;">
                    <thead><tr><th>NO</th><th>NOMOR/KODE SURAT</th><th>TGL SURAT</th><th>KEPADA</th><th>PERIHAL</th><th>LAMPIRAN</th><th>TEMBUSAN</th></tr></thead>
                    <tbody>
